...
private void nextLine() {
    try {
        ...
        try { this.input.close(); } catch (IllegalStateException ignored) {}
        this.input = null;
        this.currentLine = "";
        this.charsScanned = 0;
        ...
        while (true) {
            ...
            if (c == '\\') {
                ...
                switch (e) {
                    ...
                    case 't': builder.append('\t'); advance(2); break;
                    case 'r': builder.append('\r'); advance(2); break;
                    case '\\': builder.append('\\'); advance(2); break;
                    case '\'': builder.append('\''); advance(2); break;
                    case '"': builder.append('"'); advance(2); break;
                    case '0': builder.append('\0'); advance(2); break;
                    default:
    ...
    private void nextCharLiteral() {
        ...
        if (cur() == '\\') {
            ...
            switch (e) {
                case 'n': value = '\n'; advance(2); break;
                case 't': value = '\t'; advance(2); break;
                case 'r': value = '\r'; advance(2); break;
                case '\\': value = '\\'; advance(2); break;
                case '\'': value = '\''; advance(2); break;
                case '"': value = '"'; advance(2); break;
                case '0': value = '\0'; advance(2); break;
                default:
    ...
    private void nextIntLiteral() {
        ...
        if (cur() == '0') {
            ...
            if (!atEOL() && Character.isDigit(cur())) {
                ...
                System.out.println(RED + "ERROR: " + RESET + "Invalid decimal with leading zero: `" + bad + "` at line " + lineIdx);
                currentToken = SHC.ERROR;
                return;
            }
            latestInt = 0;
        ...
        advance(1);
        while (!atEOL() && Character.isDigit(cur())) advance(1);

        String lex = currentLine.substring(start, charsScanned);
        try {
            long v = Long.parseLong(lex, 10);
            if (v > Integer.MAX_VALUE) throw new NumberFormatException();
            latestInt = (int) v;
            currentToken = SHC.INT_LITERAL;
    ...
    private void nextIdentifierOrKeyword() {
        ...
        advance(1);
        while (!atEOL() && isIdentPart(cur())) advance(1);
        ...
        for (int i = 0; i < KEYWORDS.length; i++) {
            if (token.equals(KEYWORDS[i])) {
                ...
                if (currentToken == SHC.TRUE) { latestInt = 1; } // optional value
                if (currentToken == SHC.FALSE) { latestInt = 0; }
                return;
            }
        }
        // identifier
    ...
    public void nextToken() {
        ...
        if (!tryNextSymbol()) {
            if (atEOL()) { nextLine(); nextToken(); return; }
    ...
    public SHC currentToken() { return this.currentToken; }

    public String getId() { return this.latestIdentifier; }

    public int getInt() { return this.latestInt; }

    public int getChar() { return this.latestChar; }

    public String getString() { return this.latestString; }

    public String getCurrentTokenLine() { return currentTokenLine; }

    public int getLineIdx() { return lineIdx; }

    public int getCharIdx() { return charIdx; }

    public int getEndingLineIdx() { return linesScanned; }

    public int getEndingCharIdx() { return charsScanned; }
