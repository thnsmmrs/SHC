public final class ASTPrinter {
  ...
  private static void printFunction(Function f, StringBuilder sb) {
    ...
    for (Variable v : f.getArguments())
      sb.append("    ").append(v.getType()).append(stars(v.getNReferences())).append(" ").append(v.getName())
          .append("\n");
    sb.append("  locals:\n");
  ...
  private static void printStmt(String ind, Statement s, StringBuilder sb) {
    if (s instanceof Statement.If iff) {
      ...
      for (Statement t : iff.body())
        printStmt(ind + "  ", t, sb);
      sb.append(ind).append("}\n");
      if (iff.otherBody().length > 0) {
        sb.append(ind).append("else {\n");
        for (Statement t : iff.otherBody())
          printStmt(ind + "  ", t, sb);
        sb.append(ind).append("}\n");
      }
  ...
  private static String printExpr(Expression e) {
    Assignment a = e.getExpressions()[0];
    return printAdd(a.getValue().getRight().getRight().getRight().getRight());
  }

  private static String printAdd(AdditiveExpression a) {
    if (!a.hasLeft())
      return printMul(a.getRight());
    return "(" + printAdd(a.getLeft()) + " " + a.getOperator() + " " + printMul(a.getRight()) + ")";
  }

  private static String printMul(MultiplicativeExpression m) {
    if (!m.hasLeft())
      return printUnary(m.getRight());
    return "(" + printMul(m.getLeft()) + " " + m.getOperator() + " " + printUnary(m.getRight()) + ")";
  }

  private static String printUnary(UnaryExpression u) {
    if (u.hasOperator())
      return "(" + u.getOperator() + printUnary(u.getUnaryExpression()) + ")";
    return printFactor(u.getFactor());
  }

  private static String printFactor(Factor f) {
    if (f instanceof Factor.Const c)
      return Integer.toString(c.constant());
    if (f instanceof Factor.Str s)
      return "\"" + s.string() + "\"";
    if (f instanceof Factor.Parentheses p)
      return "(" + printExpr(p.expression()) + ")";
    if (f instanceof Factor.Var v)
      return v.toString(); // uses Variable.toString()
